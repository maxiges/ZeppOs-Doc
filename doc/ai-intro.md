...
- [ZML](zml/README.md) - The Zepp Mobile Library (ZML). Refer to this documentation when implementing bidirectional communication between the watch and phone (`app-side` vs `page`), handling asynchronous data requests, or using `BasePage` / `BaseSideService` wrappers. Further details are available at the [official ZML repository](https://github.com/zepp-health/zml).
